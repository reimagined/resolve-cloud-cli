...
test('command', () => {
  expect(command).toEqual('enable <deployment-id>')
...
describe('handler', () => {
  ...
  test('api call', async () => {
    ...
    expect(patch).toHaveBeenCalledWith('token', 'deployments/deployment-id/tracing/enable', {})
  })
})
