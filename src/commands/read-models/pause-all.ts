...
export const handler = refreshToken(async (token: any, params: any) => {
  ...
  for (const { eventSubscriber } of result) {
    promises.push(
      Promise.resolve()
        .then(() =>
          patch(token, `deployments/${deploymentId}/read-models/${eventSubscriber}/pause`, {})
        )
...
export const command = 'pause-all <deployment-id>'
export const describe = chalk.green('pause updates for all read models')
